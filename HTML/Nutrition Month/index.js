...
const Game = {
	...
	startGame: function () {
		...
		if (Game.elements.content) {
			Game.elements.content.classList.add('is-hidden');
	}

		Game.elements.bg.classList.add('is-blurred');
...
const menuStatics = [
	Bodies.rectangle(Game.width / 2, Game.height * 0.90, 512, 96, {
		isStatic: true,
		label: 'btn-start',
...
const resizeCanvas = () => {
	// The logical game size (Game.width/Game.height) was set to the
	// window's size at load, so we just stretch the canvas/UI to fill
	// the viewport 1:1 — no letterboxing, no scaling math needed.
	...
	Game.elements.ui.style.transform = 'scale(1)';
};
